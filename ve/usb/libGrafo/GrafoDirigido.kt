...
/*
    ...
    Para las listas de adyacencias se usaran MutableList<Arco>
...
public class GrafoDirigido : Grafo {
    val listasAdy: MutableList<MutableList<Arco>>
    val listaArcos: MutableList<Arco>
    val nVertices: Int
    var nLados: Int
    ...
    constructor(nombreArchivo: String)  {
        ...
        for (i in 2 until infoArchivo.size) {
            ...
            this.agregarArco(Arco(u, v))
        }
    }

    /* 
        Se agrega un arco al digrafo.

        Si el arco no se encuentra en el grafo, se agrega y se devuelve true.
        Se devuelve false caso contrario

        {P: Los vertices del arco pertenecen al grafo}
        {Q: E = union(E, arco nuevo)}
    ...
    */
    fun agregarArco(a: Arco) : Boolean {
        if (a.fuente() < 0 || a.fuente() > nVertices) {
            throw RunTimeException("GrafoDirigido.agregarArco(): El vertice ${a.fuente()} no pertenecen al grafo")
        }

        if (a.sumidero() < 0 || a.sumidero() > nVertices) {
            throw RunTimeException("GrafoDirigido.agregarArco(): El vertice ${a.sumidero()} no pertenece al grafo")
        }
    ...
    */
    ...
    /* 
        ...
        input: v -> Entero que repreeseenta un del grafo
        Output: Los arcos eque tengaen como fuente a v
    */
    ...
    override operator fun iterator() : Iterator<Arco> = listaArcos.iterator()
     
    /* 
        Se devuelve la representacion como string del grafo
    */
